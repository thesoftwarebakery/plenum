...
///
/// Resolution order:
///   1. `OPENGATEWAY_NODE_SERVER` env var (override for testing / custom deployments)
...
pub fn locate_server_script() -> Result<PathBuf, Box<dyn Error + Send + Sync>> {
    // 1. Explicit override.
    if let Ok(path) = std::env::var("OPENGATEWAY_NODE_SERVER") {
        let p = PathBuf::from(&path);
        if p.exists() {
            return Ok(p);
        }
        return Err(format!(
            "OPENGATEWAY_NODE_SERVER is set to '{path}' but the file does not exist"
        )
        .into());
    }
